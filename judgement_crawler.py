...
def extract_hidden_vals_from_soup(detail_soup):
    """
    detail.do AJAX 응답 HTML에서 초심보기 클릭 시 필요한 hidden input 값 추출.

    detailClick('JR') 함수가 읽는 DOM hidden inputs:
      - medi_numb : 초심사건번호
      - begi_orga : 초심 위원회 코드
      - even_numb : 재심사건번호 (현재 팝업)
      - even_gubn : JS (재심 고정)
      - comm_code : 위원회 코드
      - midd_rscd : 결과 코드

    ★ 이 값들을 재심 AJAX 응답에서 파싱해두고,
      배경 요청으로 DOM이 오염되기 전에 강제로 덮어써야 초심보기가 올바른 사건을 요청함.
    """
...
async def get_initial_case_by_clicking(page, hidden_vals):
    """
    '초심보기' 버튼을 클릭해 초심사건 상세 정보를 가져온다.

    ★ 문제 원인 (테스트로 확인):
      재심 팝업이 열린 직후 페이지 JS가 자동으로 /detail.do 배경 요청을 발생시켜
      DOM hidden inputs(even_numb, medi_numb 등)을 stale 데이터로 덮어씀.
      → 초심보기 클릭 시 detailClick('JR')가 잘못된 사건번호를 읽어 전송.

    ★ 해결 방식 (DOM 강제 덮어쓰기 + expect_response):
      1) 재심 AJAX 응답 HTML에서 파싱한 hidden_vals를 DOM에 강제 설정
         (배경 요청이 덮어쓴 값을 올바른 재심 값으로 복원)
      2) page.expect_response("/detail.do")로 다음 AJAX 응답 대기 등록
      3) 초심보기 버튼 JS 클릭 → detailClick('JR')이 올바른 값 읽어 요청 전송
      4) 캡처된 AJAX 응답 HTML을 파싱 → 위원회 + 판정내용 추출
    ...
    """
    try:
        # Step 1: DOM hidden inputs 강제 덮어쓰기 (배경 요청 오염 복원)
        ...
        """, hidden_vals)
        print(f"   🔧 DOM hidden inputs 강제 설정: {set_count}개 필드")

        # Step 2: 초심보기 버튼 존재 확인
        ...
        # Step 3: expect_response 등록 후 클릭 → AJAX 응답 캡처
        print("   🖱️ 초심보기 클릭 (expect_response 방식)...")
        ...
        print(f"   ✅ 초심 AJAX 응답 캡처 ({len(initial_content)}자)")
                ...
                try:
                    ...
                    if '중앙' in item['committee']:
                        ...
                        if initial_case_number:
                            # ★ 핵심: 배경 요청이 DOM을 오염시킬 수 있으므로 2초 대기
                            # (테스트 확인: 재심 팝업 직후 자동 /detail.do 배경 요청 발생)
                            print("   ⏳ 배경 요청 안정화 대기 (2초)...")
                            await asyncio.sleep(2)

                            # ★ DOM 강제 덮어쓰기 + expect_response 방식으로 초심 AJAX 캡처
                            initial_data = await get_initial_case_by_clicking(page, hidden_vals)
